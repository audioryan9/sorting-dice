...
const showMeTheCube = (c) => `<div class="cube"><span>${getVal(c.num)}</span></div>`;
document.querySelector('#roll').addEventListener('click', () => {
    dice = getDice(document.querySelector('#amount').value);
 document.querySelector('.unsortedDice').innerHTML = dice.map(c => showMeTheCube(c)).join('');
 document.querySelector('.solution-log').innerHTML = '';
});
document.querySelector('#sort').addEventListener('click', () => {
 selection(dice);
 document.querySelector('.sortPath').innerHTML = log.map((iter,i) => `<li><i>${i}</i><div class="cube">${iter.map(c => showMeTheCube(c)).join('')}</div></li>`).join('');
});